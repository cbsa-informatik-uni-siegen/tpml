...
\begin{enumerate}
   ...
   \statement{Nennen Sie Fehlerquellen der DAUs}{\page{219}}
  {
   ...
   \statement{Nennen Sie drei verschiedene geläufige Verfahren der DAU}
   {\pages{219}{}}
  {
   ...
   \statement{Beschreiben Sie die prinzipielle Funktionsweise des 
   Stromsummenwandlers}{\page{219}}
  {
    \catchword{Referenzspannung wird mittels verschiedener Widerstände in sich 
    um den FAktor 2 unterscheidende Ströme aufgeteilt}
    \catchword{Die zu den Digitalsbits gehörenden Ströme werden aufsummiert und 
    ergeben den Ausgangsstrom}
}
  ...
  {
    \normaltext{Da im Multiplexverfahren die Messsignale nacheinander umgewandelt werden, können unter Umständen
    zeitliche Beziehungen verloren gehen; z.B. wenn man die WErt von verschiedenen Messstellen für exakt den selben
    Zeitpunkt wissen möchte.}
  }

\statement{Skizzieren Sie das Blockschaltbild des Sägezahnumsetzers}{\page{207}}
  {
    siehe Bild 3.28, Seite 207
  }
  ...
  \statement{Erklären Sie detailiert den Integrationswandler nach dem Zwei-Rampen-Verfahren}{\pages{208}{210}}
  {
    ...
    das Messsignal über einen festen Zeitraum $t_1 = N*T$ integriert, welcher durch einen zentralen 
    Takt bestimmt wird. Dabei wird ein Kondensator C auf eine Spannung $U_C$ aufgeladen, die exakt
    dem Mittelwert der Messspannung $U_x$ während der Zeit $t_1$ entspricht. Anschließend wird der
    Kondensator durch die Bezugsspannungsquelle $U_R$ entladen, und die dazu benötigte Zeit
    $t_2 = x*T$ durch mitzählen der Taktimpulse gemessen. Man erhält daraus den Messwert 
    $U_x = U_R * x/N$ wobei sich alle übrigen Gerätegrößen (R, C, T) herausheben, wenn sie während
    der Messzeit konstant sind. Dadurch wird dieses Verfahren sehr genau.
    Auch die Auflösung ist sehr gut, wenn man mit einer genügend hohen Taktfrequenz arbeitet,
    typischerweise 1MHz, die durch den DAU begrenzt wird. 
    Die Besonderheit bei diesem Verfahren liegt darin, dass durch die Integration ein Mittelwert
    über einen größeren Zeitraum gebildet wird, und deabei Schankungen und Störungen des Messsignals
    herausgemittelt werden.
    Da kleinen Signalen oft eine Störung durch das Versorgungsnetz mit 50 Hz überlagert ist, kann man
    mit Messzeiten, die ein Vielfaches der Periodendauer von 20 msec sind, diese Störungen weitgehend
    unterdrücken. Für Momentanwertmessungen muss hier immer ein Sample-an-Hold-Glied eingesetzt werden.}
  }
       ...
       \statement{Nennen Sie den Vorzteil des Stromsummenwandlers und beschreiben Sie genau sein Problem}{\page{220}}
  {
    ...
    und 0111 1111, die sich nur um ein Bit unterscheiden. Im ersten Fall wird der Strom nur duch einen
    Wiederstand mit 1kOhm fließen (Strom ist dann 5mA), im zweiten Fall durch alle anderen parallel fließen:
    Die Summe wäre 254/265*5mV = 4,98mA. Es reicht aus, wenn der erste Widerstand um mehr als 4 Ohm von
    seinen 1kOhm abweicht und der Strom würde kleiner als 4,8 mA, und damit läge ein Monotoniefehler vor.}
  }
       ...
       \statement{Skizzieren Sie den prinzipiellen Aufbau des R-2R-Widerstandnetzwerkes. )}{\page{221}}
  {
    \normaltext{Siehe Bild}
        ...
        \statement{Skizzieren Sie den prinzipiellen Aufbau des Linear-Umsetzers}
        {\page{222}}
  {
    \normaltext{Siehe Bild}
         ...
         \statement{Nennen Sie die entscheidnenden Vor- und Nachteile des 
         Linear-Umsetzers}{\page{222}}
  {
    \catchword{Nachteil: riesiger Aufwand: für n Bits braucht man $2^n$ 
    Wiederstände und $(2^n)-1$ Umschalter}
  ...
  \statement{Erklären Sie das Prinzip der Mehrheitsentscheidung}{\page{250}}
  {
   ...
   \statement{Nennen Sie drei Verfahren um Eingangspinns bzw. Ausgangspins an 
   dem Rechner spraren zu können}{\pages{226}{227}}
  {
    \catchword{Multiplexer}
    \catchword{Kontaktmatrix, Relaismatrix}
    \catchword{Parallelle Daten in Serielle Daten umwandeln: UARTS (Siehe S. 
    227}
  }

  \statement{Beschreiben Sie den Aufbau der Kontaktmatrix (Rlaismatrix) und
  Szizzieren Sie diese}{\page{226}}
  {
    \normaltext{Raster von Abfragekontakten, die durch Prozessgrößen (z.B. auch 
    die Zeit) betättigt werden. Über diese Kontaktpunkte wird jeweils einer der
    Leitungen an eine der Eingangsleitungen durchgeschaltet.}
  }
  
    \question{Welchen entscheidenden Nachteil haben UARTS}{\page{226}}
  {
    \normaltext{Durch die Seriallisierung wird die Datenübertragung verlangsamt
    und verzögert.}
  }
  
    \statement{Beschreiben Sie die Funktionsweise und Anforderungen an die 
    Multiplexverfahren}{\page{226}}
  {
    \catchword{Zeitmultiplexverfahren: die verschiedenen Daten werden in 
    reservierten Zeitabschnitten übertragen. In der Zeit, in der Umgeschaltet 
    wird darf sich das Signal nicbt ändern, es muss also gepuffert werden. 
    Sender und Empfänger müssen synchron arbeiten.}
    \catchword{Frequenzmultiplexverfahren: Jeder Kanal wird einer anderen 
    Trägerfrequenz aufmodelliert und nach der Übertragung durch Filter wieder
    getrennt. Die Bänder dürfen sich nicht überlagern. }
  }
