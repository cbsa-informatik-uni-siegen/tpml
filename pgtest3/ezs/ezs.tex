...
\begin{enumerate}
  ...
  \question{Wie ist der Begriff Prozess definiert}{\page{6}}
  {
    ...
    Energie oder Information. Ein technischer Prozess ist ein Prozess, dessen Zustandsgrößen
  ...
  \statement{Nennen Sie die Ereignisse die in der Zentraleinheit selber auftauchen}{\page{134}}
  {
    \catchword{HALT oder RESET: Funktionen, die als nicht maskierbare Interrupts behandelt werden}
    \catchword{Impulse von der Echtzeituhr: }
    \catchword{Spannungsausfall}
    \catchword{Ablauffehler bei einer Instruktion}
  }

  \question{Was versteht man unter Supervisor Calls}{\page{135}}
  {
    \normaltext{Unter Supervisor Calls versteht man Software-induzierte Interrupts}
  }

  \question{Welche zwei Arten von Ablauffehlern können auftreten}{\page{139}}
  {
    \catchword{Programmfehler: Versteckte Fehler im Programm}
    \catchword{Hardware-Fehler: Permanenter Ausfall von Bauteilen oder temporäre Störung}
  }

  \question{Wie kann der Rechner unterschiedliche Fehler erkennen}{\page{139}}
  {
    \normaltext{Indem der Interrupt einen Vektor zur Verfügung stellt}
  }

  \statement{Nennen und erklären Sie kurz die auftretenden Fehler}{\pages{140}{141}}
  {
    \catchword{Time-out: Die adressierte Speicherstelle meldet sich nicht}
    \catchword{Odd address: Fehler aufgrund von fehlerhaften Wortgrenzen}
    \catchword{Parity: Fehler aufgrund von falschen Prüfbits}
    \catchword{Space violation: Bereichsverletzung festgestellt wurch die MMU}
    \catchword{Illegal instruction: Der Instruktionscode ist nicht interpretierbar}
    \catchword{Reserved instruction: Resevierte Instruktionscodes verwendet}
    \catchword{Stack limit violation: Stack ist übergelaufen}
    \catchword{Illegal operand: Operanden passen nicht zur Operation}
    \catchword{Illegal jump: Falsches Verzweigungsziel}
    \catchword{Overflow: Überlauf eines Registers}
