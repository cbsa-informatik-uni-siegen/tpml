 ...
 */
public abstract class AbstractMinimalTypingProofRuleSet extends AbstractProofRuleSet {
  //
  // Constructor (protected)
  //
  
  /**
   * Allocates a new <code>AbstractMinimalTypingProofRuleSet</code> for the specified <code>language</code>.
   * 
   * @param language the {@link Language} to which the minimal typing proof rules in this set belong.
   * 
  ...
  protected void register(int group, String name, final Method applyMethod, final Method updateMethod) {
    ...
    register(new AbstractMinimalTypingProofRule(group, name) {
      @Override
      protected void applyInternal(MinimalTypingProofContext context, MinimalTypingProofNode node) throws Exception {
        applyMethod.invoke(AbstractMinimalTypingProofRuleSet.this, context, node);
      }
      @Override
      protected void updateInternal(MinimalTypingProofContext context, MinimalTypingProofNode node) throws Exception {
        if (updateMethod != null) {
          updateMethod.invoke(AbstractMinimalTypingProofRuleSet.this, context, node);
        }
      }
    });
  }
  ...
  private Method getMethodByName(String methodName) {
    ...
    try {
      ...
      return getClass().getMethod(methodName, new Class[] { MinimalTypingProofContext.class, MinimalTypingProofNode.class });
    }
