...
public class L1TypeCheckerProofRuleSet extends AbstractTypeCheckerProofRuleSet {
	...
	public void applyAbstr(TypeCheckerProofContext context,
		...
		if (expression instanceof Lambda) {
			...
			context.addProofNode(node, environment.extend(lambda.getId(), tau1),
					lambda.getE(), tau2);
		}
		else {
			// determine the type for the parameter
	...
	public void applyApp(TypeCheckerProofContext context,
		...
		try {
			...
			context.addProofNode(node, node.getEnvironment(), application.getE1(),
					tau1);
			context.addProofNode(node, node.getEnvironment(), application.getE2(),
					tau2);
		}
	...
	public void applyCond(TypeCheckerProofContext context,
		...
		context.addProofNode(node, node.getEnvironment(), condition.getE0(),
				new BooleanType());
		context.addProofNode(node, node.getEnvironment(), condition.getE1(), node
				.getType());
		context.addProofNode(node, node.getEnvironment(), condition.getE2(), node
				.getType());
	}
	...
	public void applyLet(TypeCheckerProofContext context,
		...
		if (expression instanceof Let) {
			...
			context.addProofNode(node, environment.extend(let.getId(), tau1), let
					.getE2(), node.getType());
		}
			...
			context
					.addProofNode(node, environment2, multiLet.getE2(), node.getType());
			...
			// add the child nodes
			context.addProofNode(node, environment, e1, tau1);
			context.addProofNode(node, environment.extend(identifiers[0], tau1),
					curriedLet.getE2(), node.getType());
		}
	}
