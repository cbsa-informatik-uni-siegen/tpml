 ...
 */
public final class DefaultTypeEnvironment extends AbstractEnvironment < Identifier, Type > implements TypeEnvironment {
	/**
	...
	public TypeEnvironment substitute ( TypeSubstitution s ) {
		...
		for ( Mapping < Identifier, Type > mapping : this.mappings ) {
			...
			if ( !newType.equals ( mapping.getEntry ( ) ) ) {
				environment.mappings.add ( new Mapping < Identifier, Type > ( mapping.getSymbol ( ), newType ) );
			} else {
