...
public final class TypeEquationListTypeChecker {
	 ...
	 */
	private TypeEquationListTypeChecker ( TypeEquationTypeChecker pFirst, TypeEquationListTypeChecker pRemaining ) {
		if ( pFirst == null ) {
	 ...
	 *           is <code>null</code>.
	 */
	public TypeEquationListTypeChecker extend ( TypeEquationTypeChecker pTypeEquationTypeChecker ) {
	...
	public TypeEquationListTypeChecker substitute ( TypeSubstitution s ) {
		...
		return new TypeEquationListTypeChecker ( this.first.substitute ( s ), this.remaining.substitute ( s ) );
	}
	...
	public DefaultTypeSubstitution unify ( ) throws UnificationException {
		...
		if ( this.first.getSeenTypes ( ).contains ( this.first ) ) {
			Debug.out.println ( "Unify - ASSUME      " + left + " = " + right, Debug.CHRISTIAN ); //$NON-NLS-1$//$NON-NLS-2$
			return this.remaining.unify ( );
		}
		// TRIV
		else if ( left.equals ( right ) ) {
			Debug.out.println ( "Unify - TRIV        " + left + " = " + right, Debug.CHRISTIAN ); //$NON-NLS-1$//$NON-NLS-2$
		...
		// MU-LEFT
			...
			Debug.out.println ( "Unify - MU-LEFT     " + left + " = " + right, Debug.CHRISTIAN ); //$NON-NLS-1$//$NON-NLS-2$
			TypeEquationListTypeChecker eqns = this.remaining;
			eqns = eqns.extend ( new TypeEquationTypeChecker ( recType.getTau ( ).substitute ( recType.getTypeName ( ),
					recType ), right, this.first.getSeenTypes ( ).clone ( ) ) );
			return eqns.unify ( );
		}
		// MU-RIGHT
		else if ( right instanceof RecType ) {
			RecType recType = ( RecType ) right;
			Debug.out.println ( "Unify - MU-RIGHT    " + left + " = " + right, Debug.CHRISTIAN ); //$NON-NLS-1$//$NON-NLS-2$
			TypeEquationListTypeChecker eqns = this.remaining;
			eqns = eqns.extend ( new TypeEquationTypeChecker ( left, recType.getTau ( ).substitute (
					recType.getTypeName ( ), recType ), this.first.getSeenTypes ( ).clone ( ) ) );
			return eqns.unify ( );
		}
		// VAR
			...
			Debug.out.println ( "Unify - VAR         " + tvar + " = " + tau, Debug.CHRISTIAN ); //$NON-NLS-1$//$NON-NLS-2$
			...
			throw new RuntimeException ( MessageFormat
					.format ( Messages.getString ( "UnificationException.1" ), tvar, tau ) ); //$NON-NLS-1$
		}
		// ARROW
		else if ( ( left instanceof ArrowType ) && ( right instanceof ArrowType ) ) {
			Debug.out.println ( "Unify - ARROW       " + left + " = " + right, Debug.CHRISTIAN ); //$NON-NLS-1$//$NON-NLS-2$
			...
			TypeEquationListTypeChecker eqns = this.remaining;
			SeenTypes < TypeEquationTypeChecker > seenTypes1 = this.first.getSeenTypes ( ).clone ( );
			seenTypes1.add ( this.first );
			eqns = eqns.extend ( new TypeEquationTypeChecker ( taul.getTau2 ( ), taur.getTau2 ( ), seenTypes1 ) );
			SeenTypes < TypeEquationTypeChecker > seenTypes2 = this.first.getSeenTypes ( ).clone ( );
			seenTypes2.add ( this.first );
			eqns = eqns.extend ( new TypeEquationTypeChecker ( taul.getTau1 ( ), taur.getTau1 ( ), seenTypes2 ) );
			return eqns.unify ( );
		}
		// TUPLE
		else if ( ( left instanceof TupleType ) && ( right instanceof TupleType ) ) {
			Debug.out.println ( "Unify - TUPLE       " + left + " = " + right, Debug.CHRISTIAN ); //$NON-NLS-1$//$NON-NLS-2$
			...
			if ( typesl.length == typesr.length ) {
				...
				for ( int n = 0; n < typesl.length; ++n ) {
					SeenTypes < TypeEquationTypeChecker > seenTypes = this.first.getSeenTypes ( ).clone ( );
					seenTypes.add ( this.first );
					eqns = eqns.extend ( new TypeEquationTypeChecker ( typesl[n], typesr[n], seenTypes ) );
				}
				return eqns.unify ( );
			}
			throw new RuntimeException ( MessageFormat.format (
					Messages.getString ( "UnificationException.4" ), left, right ) ); //$NON-NLS-1$
		}
		// LIST
		else if ( ( left instanceof ListType ) && ( right instanceof ListType ) ) {
			Debug.out.println ( "Unify - LIST        " + left + " = " + right, Debug.CHRISTIAN ); //$NON-NLS-1$//$NON-NLS-2$
			...
			SeenTypes < TypeEquationTypeChecker > seenTypes = this.first.getSeenTypes ( ).clone ( );
			seenTypes.add ( this.first );
			eqns = eqns.extend ( new TypeEquationTypeChecker ( taul.getTau ( ), taur.getTau ( ), seenTypes ) );
			return eqns.unify ( );
		}
		// REF
		else if ( ( left instanceof RefType ) && ( right instanceof RefType ) ) {
			Debug.out.println ( "Unify - REF         " + left + " = " + right, Debug.CHRISTIAN ); //$NON-NLS-1$//$NON-NLS-2$
		...
		// OBJECT
			...
			Debug.out.println ( "Unify - OBJECT      " + left + " = " + right, Debug.CHRISTIAN ); //$NON-NLS-1$//$NON-NLS-2$
			ObjectType tau1 = ( ObjectType ) left;
			ObjectType tau2 = ( ObjectType ) right;
			TypeEquationListTypeChecker eqns = this.remaining;
			SeenTypes < TypeEquationTypeChecker > seenTypes = this.first.getSeenTypes ( ).clone ( );
			seenTypes.add ( this.first );
			eqns = eqns.extend ( new TypeEquationTypeChecker ( tau1.getPhi ( ), tau2.getPhi ( ), seenTypes ) );
			return eqns.unify ( );
		}
		// ROW
		else if ( ( left instanceof RowType ) && ( right instanceof RowType ) ) {
			Debug.out.println ( "Unify - ROW         " + left + " = " + right, Debug.CHRISTIAN ); //$NON-NLS-1$//$NON-NLS-2$
			...
			for ( int i = tau1Identifiers.size ( ) - 1; i >= 0; i-- ) {
				for ( int j = tau2Identifiers.size ( ) - 1; j >= 0; j-- ) {
					if ( tau1Identifiers.get ( i ).equals ( tau2Identifiers.get ( j ) ) ) {
						SeenTypes < TypeEquationTypeChecker > seenTypes = this.first.getSeenTypes ( ).clone ( );
						seenTypes.add ( this.first );
						eqns = eqns.extend ( new TypeEquationTypeChecker ( tau1Types.get ( i ), tau2Types.get ( j ),
								seenTypes ) );
			...
			if ( ( tau1RemainingRow != null ) && ( tau2RemainingRow != null ) ) {
				SeenTypes < TypeEquationTypeChecker > seenTypes = this.first.getSeenTypes ( ).clone ( );
				seenTypes.add ( this.first );
				eqns = eqns.extend ( new TypeEquationTypeChecker ( tau1RemainingRow, tau2RemainingRow, seenTypes ) );
				if ( ( tau1Identifiers.size ( ) > 0 ) || ( tau2Identifiers.size ( ) > 0 ) ) {
					throw new RuntimeException ( MessageFormat.format (
							Messages.getString ( "UnificationException.3" ), left, right ) ); //$NON-NLS-1$
				}
				return eqns.unify ( );
			}
			...
			if ( tau1RemainingRow != null ) {
				if ( tau2Identifiers.size ( ) > 0 ) {
					Identifier[] newIdentifiers = new Identifier[tau2Identifiers.size ( )];
					...
					SeenTypes < TypeEquationTypeChecker > seenTypes = this.first.getSeenTypes ( ).clone ( );
					seenTypes.add ( this.first );
					eqns = eqns.extend ( new TypeEquationTypeChecker ( tau1RemainingRow, newRowType, seenTypes ) );
				}
			}
			// Only second remaining RowType
			if ( tau2RemainingRow != null ) {
				if ( tau1Identifiers.size ( ) > 0 ) {
					Identifier[] newIdentifiers = new Identifier[tau1Identifiers.size ( )];
					...
					SeenTypes < TypeEquationTypeChecker > seenTypes = this.first.getSeenTypes ( ).clone ( );
					seenTypes.add ( this.first );
					eqns = eqns.extend ( new TypeEquationTypeChecker ( newRowType, tau2RemainingRow, seenTypes ) );
				}
			}
		...
		// STRUCT
		throw new RuntimeException ( MessageFormat.format ( Messages.getString ( "UnificationException.0" ), left, right ) ); //$NON-NLS-1$
	}
}
