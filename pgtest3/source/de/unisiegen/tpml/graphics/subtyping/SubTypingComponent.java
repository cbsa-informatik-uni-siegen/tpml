...
public class SubTypingComponent extends AbstractProofComponent implements Scrollable 
{
	...
	private void checkForUserObject (SubTypingProofNode node) {
		...
		if (nodeComponent == null) {
			...
			nodeComponent.addSubTypingNodeListener(new SubTypingNodeListener () {
				...
				public void requestJumpToNode (ProofNode node) {
					SubTypingComponent.this.jumpNode = node;
				}
			});
		}
		...
		for (int i=0; i<node.getChildCount(); i++) {
			SubTypingProofNode pNode = (SubTypingProofNode) node.getChildAt(i) ;
	...
	protected void relayout() {
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run () {
				...
				Point rightBottomPos = SubTypingComponent.this.treeNodeLayout.placeNodes (rootNode, SubTypingComponent.this.border, SubTypingComponent.this.border, SubTypingComponent.this.availableWidth);
				
				// lets add some border to the space
				
				rightBottomPos.x += SubTypingComponent.this.border;
				rightBottomPos.y += SubTypingComponent.this.border;
