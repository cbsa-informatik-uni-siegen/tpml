...
public class TypeFormularRenderer extends AbstractRenderer
{
	...
	// their is no nice nail avialble at the unicode (unbelivable) so the nail is rendern manualy
	//private static final String As =  "A |- ";
	...
	/**
	 * resets the saved areas. They must be rebuild to garante assure the areas for the tooltiptexts are
	 * correct.
	 */
	...
	public Dimension getNeededSize(int maxWidth)
	{
...

		{
			...
			// we need 4 widths, but for the result we need the max of these 4 widths. With everey pass of the 
			// for iteration these values will be calculated. The result is the max of these maxes.  
			...
			for (int i = 0; i < this.typeFormulaList.size(); i++)
			{
				...
				// the rest of the width of the line is stored to decide to break or not to break
				...
				if (t instanceof TypeJudgement)
				{
				  ...
				  // if there is not enough space for the expression
					...
					{
						// if it is not warped to the next line the width of the environment will increse...
					...
					{
						// if it has never warped the widthof the expression is unchanges and the width will increase 
	...
	public void renderer(int x, int y, int width, int height, Graphics gc)
	{
		...
		if (this.typeFormulaList.size() > 0)
		{
			...
			for (int i = 0; i < this.typeFormulaList.size(); i++)
			{
				...
				{
					// save the posX where the render starts to render to find out the size later
					int oldX = posX;
					...
					typeEquationStringrenderer.render(posX, posY - AbstractRenderer.fontAscent, typeEquationSize.width,
					...
					this.rightTypePositions.set(i, new Rectangle((posX + typeEquationSize.width) - rightDim.width, posY
							- AbstractRenderer.fontHeight + AbstractRenderer.fontDescent, rightDim.width, rightDim.height));
				...
				else if (t instanceof TypeJudgement)
				{
					...
					this.environmentRenderer.renderer(posX, posY - AbstractRenderer.fontAscent, this.environmentRenderer
							.getNeededSize().width, this.environmentRenderer.getNeededSize().height, gc);
					...
					spaceToNexEntry = Math.max(0, this.environmentRenderer.getNeededSize().height);
					...
					remainingSpace -= AbstractRenderer.expFontMetrics.stringWidth(arrowString);

					ShowBonds bound = new ShowBonds();
					bound.load(expression);
					
					expressionRenderer.setPrettyString(expression.toPrettyString());
					Dimension expressionSize = expressionRenderer.getNeededSizeAll_(width - indentSpace);
					
					// if the expression dose not fit in the line
					if (remainingSpace < expressionSize.width + AbstractRenderer.expFontMetrics.stringWidth(doubleColon)) 
					{
						posX = indentSpace;
						remainingSpace = width - indentSpace;

						expressionSize = expressionRenderer.getNeededSizeAll_(remainingSpace);
						...
						expressionRenderer.render(posX, posY - AbstractRenderer.fontAscent, expressionSize.width,	
						...
						this.expressionPostitions.set(i, new Rectangle(posX, posY - AbstractRenderer.fontHeight
								+ AbstractRenderer.fontDescent, expressionSize.width, expressionSize.height));
					}
					else  //the expressions fits in the same line as the environment
					{
						expressionRenderer.render(posX, posY - AbstractRenderer.fontAscent, expressionSize.width,
								expressionSize.height, gc, bound, this.toListenForM);
					...
					remainingSpace -= AbstractRenderer.expFontMetrics.stringWidth(doubleColon);

					typeRenderer.setPrettyString(type.toPrettyString());
					ShowBonds bondType = new ShowBonds();
					bondType.load(type);

					Dimension typeSize = typeRenderer.getNeededSize(width - indentSpace);
					...
					if (remainingSpace < typeSize.width)
					{
						...
						typeRenderer.render(posX, posY - fontAscent, typeSize.width, typeSize.height, gc, bondType,
								this.toListenForM);

						startWidht = Math.max(startWidht, typeSize.width);
						startHeight += typeSize.height;
						this.typePositions.set(i, new Rectangle(posX, posY - AbstractRenderer.fontHeight
					...
					{
						typeRenderer.render(posX, posY - AbstractRenderer.fontAscent, typeSize.width, typeSize.height, gc,
								bondType, this.toListenForM);
			...
			{
				gc.drawRect(this.markedArea.x, this.markedArea.y, this.markedArea.width, this.markedArea.height);
			}
		}
	}
