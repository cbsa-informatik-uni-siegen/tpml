 ...
 * <br>
 * The <code>TypeInferenceCompnent</code> is a as complicated as the the node
 * from the Smallstepper-GUI because of the same reason: the rules applied to
 * this node are pointing on the expression of the next node.<br>
...
public class UnifyNodeComponent extends JComponent
{
   ...
   * TODO
   */
  private static final long serialVersionUID = 2268637139085783712L;


  /**
   * The origin {@link UnifyProofNode}
   */
  private UnifyProofNode proofNode;


  /**
   * The {@link TypeInferenceProofModel}
   */
  private UnifyProofModel proofModel;


  /**
   * The {@link UnifyRulesComponent} containing the Labels of already evaluated
   * Rules and the MenuButton for Rules not yet evaluated.
   */
  private UnifyRulesComponent rules;
   ...
   * {@link UnifyNodeComponent#freeUnderliningSibling(boolean, Direction)};
   * 
   * @author marcell
   */
      ...
      int pSpacing, boolean pAdvaced )
  {
          ...
          MenuButton source, final JMenuItem item )
          {
            ...
            SwingUtilities.invokeLater ( new Runnable ()
            {
              ...
              public void run ()
              {
                ...
                UnifyNodeComponent.this.menuItemActivated ( item );
    ...
    // TODO normaly nobdy needs it, but if it is removed the tooltip dose not
    // function...
  ...
  /**
   * Just paints a Rect arround the silly BopoundExpression to see failure and
   * the overlapping of the single compoundExpressions
   */
  @Override
  ...
  protected void processMouseEvent ( MouseEvent e )
  {
    ...
    try
    {
      ...
      UnifyNodeComponent nextComponent = ( UnifyNodeComponent ) node
          .getUserObject ();
  ...
  // {
  // setBounds(this.origin.x, this.origin.y, this.ruleDimension.width +
  // this.expressionDimension.width
  // + this.spacing, this.actualRuleHeight + this.actualExpressionHeight +
  // this.spacing+space);
  // }
  /**
  ...
  public void placeRules ()
  {
    ...
    // int top = getRuleTop() + (this.actualRuleHeight -
    // this.ruleDimension.height) / 2;
    int top = getRuleTop ();
  ...
  // {
  // //System.err.println("Diff"+(this.actualRuleHeight -
  // this.ruleDimension.height));
  ...
  // int top = getRuleTop() + (this.actualRuleHeight+y -
  // this.ruleDimension.height) / 2;
  // //System.err.println("top"+top);
  // //System.err.println("eigentlich:"+( getRuleTop() + (this.actualRuleHeight
  // - this.ruleDimension.height) / 2));
  // this.rules.setBounds(0, top, this.ruleDimension.width,
  // this.ruleDimension.height);
  // }
  ...
  private void freeUnderliningSibling ( boolean ignoreThis, Direction direction )
  {
    ...
    UnifyNodeComponent nextNodeComponent = ( UnifyNodeComponent ) nextNode
        .getUserObject ();
  ...
  public void placeExpression ()
  {
    if ( this.expressionDimension.height < this.actualExpressionHeight )
    {
      ...
      this.compoundExpression.setBounds ( this.ruleDimension.width
          + this.spacing, 0 + diff / 2, this.expressionDimension.width,
          this.expressionDimension.height );
    }
    else
    {
      this.compoundExpression.setBounds ( this.ruleDimension.width
          + this.spacing, 0, this.expressionDimension.width,
  ...
  private void fireNodeChanged ()
  {
    ...
    {
      if ( listeners [ i ] != UnifyNodeListener.class )
      {
        continue;
      }

      ( ( UnifyNodeListener ) listeners [ i + 1 ] ).nodeChanged ( this );
    }
  }
  ...
  private void fireRequstJumpToNode ( ProofNode node )
  {
    ...
    for ( int i = 0 ; i < listeners.length ; i += 2 )
    {
      ...
      ( ( UnifyNodeListener ) listeners [ i + 1 ] ).requestJumpToNode ( node );
    }
  }
