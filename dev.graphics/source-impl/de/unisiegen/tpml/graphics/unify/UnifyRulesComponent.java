 ...
 * <br>
 * The Rule items of a <i>UnifyRulesComponnent</i> are {@link UnifyRuleLabel}s
 * showing the rules already applied to the node, a {@link MenuButton} showing a
 * rule that has not yet been evaluated. <br>
 * The following image demonstrates a usual appearance of a
 * <code>UnifyRulesComponent</code>. <br>
 ...
 * 
 * @author michael
 * @version $Id: TypeInferenceRulesComponent.java 2796 2008-03-14 19:13:11Z
 *          fehler $
 */
public class UnifyRulesComponent extends JComponent
{

  /**
   * TODO
   */
  private static final long serialVersionUID = -518629549668463156L;
  ...
  /**
   * The list of {@link UnifyRuleLabel}s containing the information of all
   * rules that have been evaluated.
   */
  private LinkedList < UnifyRuleLabel > ruleLabels;
  ...
  /**
   * Create a new {@link UnifyRulesComponent} for the given proofNode.<br>
   * <br>
   ...
   *          informations.
   */
  public UnifyRulesComponent ( UnifyProofNode pProofNode )
  {
    this.proofNode = pProofNode;

    this.ruleLabels = new LinkedList < UnifyRuleLabel > ();
  ...
  {
    int labelHeight = Math.max ( UnifyRuleLabel.getLabelHeight (),
    ...
    for ( UnifyRuleLabel l : this.ruleLabels )
    {
    ...
    UnifyProofStep [] steps = this.proofNode.getSteps ();
    if ( steps.length > 0 )
    {

      // first reference rule will be the first node
      UnifyProofStep step = steps [ 0 ];
      int count = 1;

      for ( int i = 1 ; i < steps.length ; i++ )
      {
        UnifyProofStep cStep = steps [ i ];
        ...
        {
          UnifyRuleLabel label = new UnifyRuleLabel ( step.getRule ()
              .getName (), count );
          label.addMouseMotionListener ( adapter );
          if ( count == 1 )
          {
            //TODO: correct code according to unify
      ...
      UnifyRuleLabel label = new UnifyRuleLabel ( step.getRule ().getName (),
          count );
      label.addMouseMotionListener ( adapter );
      if ( count == 1 )
      {
