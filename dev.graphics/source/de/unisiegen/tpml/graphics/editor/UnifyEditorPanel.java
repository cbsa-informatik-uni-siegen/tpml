...
import de.unisiegen.tpml.graphics.StyledUnifyLanguageDocument;
...
public class UnifyEditorPanel extends JPanel implements EditorComponent,
    ClipboardOwner
{
  ...
  private class PopupListener extends MouseAdapter
  {
    ...
    private void maybeShowPopup ( MouseEvent e )
    {
      if ( e.isPopupTrigger () )
      {
        UnifyEditorPanel.this.popup.show ( e.getComponent (), e.getX (), e
            .getY () );
      }
    }
  ...
  private class TextDocumentListener implements DocumentListener
  {
    ...
    public void insertUpdate ( DocumentEvent arg0 )
    {
      ...
      try
      {
        UnifyEditorPanel.this.setUndoStatus ( true );
        String doctext = arg0.getDocument ().getText ( 0,
            arg0.getDocument ().getLength () );
        if ( doctext.endsWith ( " " ) )
        {
          UnifyEditorPanel.this.undohistory.push ( doctext );
          logger.debug ( "history added: " + doctext );
        }
        setRedoStatus ( false );
        UnifyEditorPanel.this.redohistory.clear ();
        UnifyEditorPanel.this.currentContent = doctext;
    ...
    public void removeUpdate ( DocumentEvent arg0 )
    {
      ...
      {
        UnifyEditorPanel.this.setUndoStatus ( true );
        UnifyEditorPanel.this.undohistory
            .push ( UnifyEditorPanel.this.currentContent );
        setRedoStatus ( false );
        UnifyEditorPanel.this.redohistory.clear ();
        UnifyEditorPanel.this.currentContent = arg0.getDocument ().getText ( 0,
            arg0.getDocument ().getLength () );
        loadOutlineExpression ( Outline.ExecuteAutoChange.EDITOR );
      }
      catch ( BadLocationException e )
   ...
   */
  static final Logger logger = Logger.getLogger ( UnifyEditorPanel.class );
  ...
  private StyledUnifyLanguageDocument document;
   ...
   */
  public UnifyEditorPanel ( Language language )
  {
    if ( language == null )
    {
  ...
  public StyledUnifyLanguageDocument getDocument ()
  {
  ...
  private void initComponents ( Language language )
  {
    ...
    this.document = new StyledUnifyLanguageDocument ( language );
    ...
    this.sideBar.addSideBarListener ( new SideBarListener ()
    {
      ...
      {
        if ( ( UnifyEditorPanel.this.editor.getSelectionStart () == pLeft )
            && ( UnifyEditorPanel.this.editor.getSelectionEnd () == pRight ) )
        {
          UnifyEditorPanel.this.removeSelectedText ();
        }
        else
        {
          UnifyEditorPanel.this.selectErrorText ( pLeft, pRight );
        }
      }
      ...
      public void insertText ( int pIndex, String pInsertText )
      {
        ...
        try
        {
          while ( UnifyEditorPanel.this.document.getText ( pIndex + countSpaces,
              1 ).equals ( " " ) ) //$NON-NLS-1$
          {
            countSpaces++ ;
          }
        ...
        try
        {
          ...
          UnifyEditorPanel.this.document.insertString ( pIndex + offset, text,
              null );
        }
        catch ( BadLocationException e )
        {
      ...
      public void replaceText ( int [] pStart, int [] pEnd, String pReplaceText )
      {
        ...
        for ( int i = 0 ; i < pStart.length ; i++ )
        {
          try
          {
            ...
            UnifyEditorPanel.this.document.replace ( offset + pStart [ i ],
  ...
  protected void loadOutlineExpression ( Outline.Execute pExecute )
  {
    ...
    try
    {
      this.outline.load ( this.document.getTypeEquation (), pExecute );
    }
    catch ( Exception e )
    {
