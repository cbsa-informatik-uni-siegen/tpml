...
public class FileWizardLogic
{
  ...
  /**
   * this methode dose the job. first it fills up the jTree with the languages
  ...
  public void getLanguages ()
  {
    ...
    for ( Language language : available )
    {
      ...
      // if the catogory is the same as the last one only add the new entry
      {
        DefaultMutableTreeNode add = new DefaultMutableTreeNode ( name );
        // put it to the last leaf
        ( ( DefaultMutableTreeNode ) root.getLastLeaf ().getParent () )
            .add ( add );
      }
    }

    // set the build root to the root of the File Wizard
    ( ( DefaultTreeModel ) this.fileWizard.languagesTree.getModel () )
        .setRoot ( root );

    // makes the tree loooking nice
    DefaultTreeCellRenderer cr = ( ( DefaultTreeCellRenderer ) this.fileWizard.languagesTree
        .getCellRenderer () );
    cr.setIcon ( null );
    cr.setLeafIcon ( null );
    cr.setOpenIcon ( null );
    ...
    // expand all expanded last expanded. The first time the programm is started
    // everey node will be expanded.
    int i = 0;
    while ( i < this.fileWizard.languagesTree.getRowCount () )
    {
      boolean expand;
      expand = this.preferences.getBoolean ( this.fileWizard.languagesTree
          .getPathForRow ( i ).toString (), true );
      if ( expand )
      {
    ...
    // with ESC the windows will be disposed, with ENTER the same will happen
    // like clicking ok
    ...
    // ((DefaultTreeModel)
    // this.fileWizard.languagesTree.getModel()).nodeChanged(root);
  ...
  TreeExpansionEvent event )
  {
    ...
    while ( i < this.fileWizard.languagesTree.getRowCount () )
    {
      ...
      // this.fileWizard.languagesTree.getPathForRow(i).getPath()).is
      expand = this.fileWizard.languagesTree.isExpanded ( i );
      this.preferences.putBoolean ( this.fileWizard.languagesTree
          .getPathForRow ( i ).toString (), expand );
      i++ ;
    }
   ...
   * React on ENTER and ESC. If the key is Enter and a languages is selected,
   * the window will dispose. If the key is ESC, the selected language will be
   * reset to null and the window will dispose.
   * 
   * @param event
   */
   ...
   * If a doubleclick is recognized the window will dispose if a language is
   * selected.
   * 
   * @param event
   */
   ...
   * If a element is selected and it is a language, the language is set. The
   * selected element of the tree is saved.
   * 
   * @param event
   */
  protected void treeSelectionHanlder ( TreeSelectionEvent event )
  {
        ...
        && ( ( DefaultMutableTreeNode ) tp.getLastPathComponent () ).isLeaf () )
    {
      ...
      if ( newLanguage != null )
      {
        ...
        this.fileWizard.descriptionTextArea.setText ( this.fileWizard.language
            .getDescription () );
