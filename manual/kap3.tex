...
\chapter{Die Sprachen im Detail}
...
Auflistung des Regelwerks darstellt, welches allerdings nicht hunderprozentig mit dem Vorlesungsinhalt "ubereinstimmt.}.
...
Die Sprache \LZERO\ stellt die einfachste denkbare Programmiersprache dar und entspricht dem reinen ungetypten $\lambda$-Kalk"ul
\engl{pure untyped $\lambda$-calculus}. Die abstrakte Syntax enth"alt lediglich drei Produktionen.
\bgram
e \is \id
  \al \abstr{id}{e}
  \al e_1 e_2
\egram
Ein g"ultiger Ausdruck ist also entweder ein Bezeichner, eine $\lambda$-Abstraktion oder eine Applikation. Die Menge \notation{$\Val$}\
$\sleq \Exp$ der \Define{Werte}{Wert} \Engl{values}{value} \notation{$\name v$} wird durch
\bgram
v \is \id
  \al \abstr{id}{e}
\egram
...
Ein big step hei"st g"ultig f"ur \LONE, wenn er sich mit den big step Regeln von \LZERO, den Regeln\\[5mm]
  \begin{tabular}{ll}
...
Ein small step hei"st g"ultig f"ur \LONE, wenn er sich mit den small step Regeln von \LZERO, den Regeln\\[5mm]
  \begin{tabular}{ll}
     ...
     \mbox{(LET-EXEC)}      & $\bli{\id}{v}{e} \to e[v/\id]$
  \end{tabular}\\[7mm]
...
\LONE\ verf"ugt "uber ein einfaches Typsystem, benutzt aber wie alle folgenden Sprachen schon den Typinferenzalgorithmus,
was anfangs vielleicht zu schwer verst"andlichen Fehlermeldungen f"uhren kann. In diesem Fall sollte es in der "Ubung
angesprochen werden.
...
als \define{Typumgebung} \engl{type environment} bezeichnet. Ein Typurteil hei"st g"ultig f"ur \LONE, wenn es sich mit
den Regeln\\[3mm]
\begin{tabular}{ll}
  ...
  \mbox{(ABSTR)\  } & $\regel{\Tj{\Gamma[\tau/\id]}{e}{\tau'}} 
                           {\Tj{\Gamma}{\abstr{\id:\tau}{e}}{\tau\to\tau'}}$
\end{tabular}\\[7mm]
...
\end{tabular}\\[7mm]
definiert. Beim Typechecker werden diese Regeln f"ur Konstanten nicht angegeben, sondern lediglich die (CONST) Regel.


\subsection{Syntaktischer Zucker}

Die Sprache \LONE\ und alle folgenden Sprachen enthalten syntaktischen Zucker um das Schreiben von Programmen zu vereinfachen.
Der syntaktische Zucker l"asst sich dann anschlie"send in Kernsyntax "ubersetzen oder direkt verarbeiten. Hierbei wurden aus
Gr"unden der "Ubersichtlichkeit keine eigenen Regeln f"ur den syntaktischen Zucker eingef"uhrt, sondern es wird implizit eine
Konvertierung des betreffenden Teilausdrucks in Kernsyntax vorgenommen. F"ur \LONE\ betrifft dies nur Ausdr"ucke, die
Operatoren in Infixschreibweise enthalten. Es gilt:
\beqns
...
es erlaubt Funktionen einfacher zu definieren, "ahnlich zu OCaml. Dar"uber hinaus bietet \LTWO\ logische Operatoren
f"ur Konjunktion und Disjunktion.
...
Ein big step hei"st g"ultig f"ur \LTWO, wenn er sich mit den big step Regeln von \LONE\ und den Regeln\\[3mm]
  \begin{tabular}{ll}
...
Ein small step hei"st g"ultig f"ur \LTWO, wenn er sich mit den small step Regeln von \LONE\ und den Regeln\\[3mm]
\begin{tabular}{ll}
...
Ein Typurteil hei"st g"ultig f"ur \LTWO, wenn es sich mit den Typregeln von \LONE\ und den Regeln\\[3mm]
\begin{tabular}{ll}
...
Die Sprache \LTWO\ enth"alt weitere Abk"urzungen zu den in \LONE\ definierten. Die logischen $\mathbin{\&\&}$- und 
...
Desweiteren stehen Abk"urzungen f"ur die einfache Definition von Funktionen zur Verf"ugung.
\beqns
...
eine "Ubersetzung in Kernsyntax vorgenommen.



\section{Die Sprache \LTHREE}

Die Sprache \LTHREE\ erweitert die Sprache \LTWO\ um ein polymorphes Typsystem, sowie Tupel und Listen, f"ur die
zus"atzlich abk"urzende Schreibweisen eingef"uhrt wurden. Die Menge \notation{$\Exp$} der g"ultigen Ausdr"ucke
wird hierf"ur um die Produktionen
\bgram
...
Ein big step hei"st g"ultig f"ur \LTHREE, wenn er sich mit den big step Regeln von \LTWO\ und den Regeln\\[5mm]
  \begin{tabular}{ll}
    ...
    \mbox{(HD-EMPTY)}       & $\Hd\,[\,] \eval empty\_list$ \\[3mm]
    \mbox{(TL)}             & $\Tl\,\left(\Cons\,\left(v_1,v_2\right)\right) \eval v_2$ \\[3mm]
    \mbox{(TL-EMPTY)}       & $\Tl\,[\,] \eval empty\_list$ \\[3mm]
...
Die small step Regeln f"ur \LTHREE\ entsprechen im wesentlichen den big step Regeln. Ein small step hei"st g"ultig f"ur
\LTHREE, wenn er sich mit den small step Regeln von \LTWO\ und den Regeln\\[5mm]
  \begin{tabular}{ll}
    ...
    \mbox{(HD-EMPTY)}       & $\Hd\,[\,] \to empty\_list$ \\[3mm]
    \mbox{(TL)}             & $\Tl\,\left(\Cons\,\left(v_1,v_2\right)\right) \to v_2$ \\[3mm]
    \mbox{(TL-EMPTY)}       & $\Tl\,[\,] \to empty\_list$ \\[3mm]
...
Wie bereits erw"ahnt verf"ugt die Sprache \LTHREE\ "uber ein polymorphes Typsystem, das hei"st dass die Axiome f"ur
...
Die Sprache \LFOUR\ erweitert schlie"slich \LTHREE\ um imperative Konzepte, also Speicher, sequentielle Ausf"uhrung und
Schleifen. Die Menge $\Exp$ der g"ultigen Ausdr"ucke wird hierzu um die Produktionen
\bgram
e \is \bift{e_1}{e_2}
  \al \bwd{e_1}{e_2}
  \al e_1 \Assign e_2
