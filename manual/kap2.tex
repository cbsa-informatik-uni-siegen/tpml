...
und~\ref{Beweiswerkzeuge} genauer betrachtet. "Uber Latex Export
kann man den Quelltext oder die Beweise in eine Tex-Datei
exportieren. Mehr dazu im Kapitel~\ref{Latexexport}. Mit {\bf
...
\section {Latexexport}
"Uber den Latexexport lassen sich Quelltext und Beweisschritte in eine Tex-Datei
exportieren. Welche dann in ein pdf mit Syntaxhighlighting "uberf"uhrt werden kann.
Bei dem Quelltext gibt es die Einschr"ankung, dass nur g"ultige Ausdr"ucke exportiert
werden k"onnen. Das bedeutet, dass der Quelltext vor dem Export geparset wird, und dann
eine Expression, bzw. ein Subtype exportiert wird. Erstellt man ein pdf aus exportierten
...
Um ein pdf zu erstellen k"onnen die folgenden vier Befehle verwendet werden:\\[5mm]
\begin{tabular}{l}
...
\subsection {Der Latexexport Dialog}
...
und die Anzahl der Seiten anzugeben, da 15 Seiten voreingestellt sind. Werden weniger Seiten
ben"otigt, entstehen am Ende eine entsprechende Anzahl an weissen Seiten, welche durch die
eigene Angabe einer Seitenanzahl entfernt werden kann.\\
Leider ist es nicht m"oglich ein Dokument
in Baumstruktur "uber Latex in ein pdf zu konvertieren, welches die Grenze von 15 Seiten 
