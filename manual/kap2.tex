...
Jeder der Beweismethoden hat f"ur den Benutzer eine weitere Komfort-Funktion eingebaut. Bei gr"o"seren Programmen, die bewiesen werden sollen, uns insbesondere, wenn der gleiche Identifier doppelt benutzt wird verliert man schnell die "Ubersicht. Folgendes Beispiel soll dies verdeutlichen:

{\bf let x =(lambda x.x x)in x x}

Wie man leicht sieht ist der Identifier "`x"' doppelt verwendet. In noch gr"o"seren Beispielen w"are es nicht mehr einfach zu "uberschauen, an welches "`x"' welches gebunden ist. Um dies zu vereinfachen kann man einfach mit dem Mauszeiger "uber eins der "`x"' fahren, und die dazugeh"origen "`x"' werden farbig hervorgehoben. Zus"atzlich wird der Identifier, an den die anderen gebunden sind extra durch eine andere Farbe hervorgehoben. Im gegebenen Beispiel würde das "`x"' nach dem "`let"' zusammen mit den letzten beiden "`x"' hervorgehoben, w"arend das "`x"' nach dem "`lambda"' zusammen mit den folgenden beiden "`x"' hervorgehoben w"urden. 
