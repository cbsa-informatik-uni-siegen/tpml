...
/**
 * The layouting of the SmallStep-GUI is a bit more complicated as the layouting
 * of the other two GUIs because of two reasons.<br>
 * <code>First</code>: One node within the <i>SmallStep-Tree</i> actualy
 * needs to rows to get placed and<br>
 ...
 */
public class SmallStepComponent extends AbstractProofComponent implements
    Scrollable, Cloneable
{
   ...
   * ScrollPane of {@link SmallStepView}.
   */
  ...
  void setAdvanced ( boolean pAdvanced )
  {
    ...
    if ( this.advanced != pAdvanced )
    {
      ...
      while ( enumeration.hasMoreElements () )
      {
        ...
        // advanced state
        SmallStepProofNode node = ( SmallStepProofNode ) enumeration
            .nextElement ();
        SmallStepNodeComponent component = ( SmallStepNodeComponent ) node
            .getUserObject ();
  ...
  /**
   * Traversing the ProofTree recursivly and adds a SmallStepNodeComponent where
   * none is.<br>
   * <br>
  ...
  void checkForUserObject ( SmallStepProofNode node )
  {
    ...
    SmallStepNodeComponent nodeComponent = ( SmallStepNodeComponent ) node
        .getUserObject ();
    if ( nodeComponent == null )
    {

      // create the noded that has not been there yet
      nodeComponent = new SmallStepNodeComponent ( node, this.model,
  ...
  Dimension placeNode ( SmallStepProofNode pNode, int pX, int pY )
  {
    ...
    while ( node != null )
    {
      ...
      else
      {
        ...
        SmallStepNodeComponent parentNodeComponent = ( SmallStepNodeComponent ) parentNode
            .getUserObject ();
   ...
   * <br>
   * All nodes in the tree will get a SmallStepNodeComponent, the size of the
   * widest rule combo on the left-hand-site is evaluated. <br>
  ...
  protected void doRelayout ()
  {
    ...
    SmallStepProofNode rootNode = ( SmallStepProofNode ) SmallStepComponent.this
    ...
    // they can be placed
    Dimension size = placeNode ( rootNode, SmallStepComponent.this
        .getThisBorder (), SmallStepComponent.this.getThisBorder () );
   ...
   * {@link #resetUserObject(SmallStepProofNode)} on the rootNode of the model.
   * <br>
   ...
   * {@link SmallStepProofModel} later.
   */
  @Override
  ...
  void jumpToNodeVisible ()
  {
    ...
    // on the viewport
    SmallStepNodeComponent node = ( SmallStepNodeComponent ) this.jumpNode
        .getUserObject ();
    if ( node == null )
    {
      return;
  ...
  {
    // if you want to see the whole small step component you might make it
    // visible here
