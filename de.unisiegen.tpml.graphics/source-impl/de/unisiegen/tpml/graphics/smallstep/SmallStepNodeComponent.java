...
public class SmallStepNodeComponent extends JComponent
{
  ...
  /**
   * Just paints a Rect arround the silly {@link CompoundExpression} to see the
   * overlapping
   */
  @Override
  ...
  // workaround: the single CompoundExpressions overlap the others. Thus it is
  // not possible to
  ...
  public void setBounds ( int space )
  {
    setBounds ( this.origin.x, this.origin.y, this.ruleDimension.width
        + this.expressionDimension.width + this.spacing, this.actualRuleHeight
        + this.actualExpressionHeight + this.spacing + space );
  }
  ...
  {
    int top = getRuleTop ()
        + ( this.actualRuleHeight - this.ruleDimension.height ) / 2 + y;
    this.rules.setBounds ( 0, top, this.ruleDimension.width,
        this.ruleDimension.height );
  }
   ...
   * with some {@link #spacing}. If the expression is lower than the rulelabels
   * the expression will be centerd.
   */
  public void placeExpression ()
  {
    if ( this.expressionDimension.height < this.actualExpressionHeight )
    {
      ...
      this.compoundExpression.setBounds ( this.ruleDimension.width
          + this.spacing, diff / 2, this.expressionDimension.width,
  ...
  /**
   * removes a {@link SmallStepNodeListener} from the listenerList of the
   * component
   * 
   * @param listener
   */
