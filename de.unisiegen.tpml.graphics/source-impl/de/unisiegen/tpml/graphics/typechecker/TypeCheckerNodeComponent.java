 ...
 * typeenvironment. Then ther will be the {@link #doubleColonLabel} containing
 * the <code>"  ::  "</code>. The last element in the first row is the
 * {@link #typeComponent} containing the resulting type. If the node is not
 * completly evaluated or the view is not in advanced mode only the four dots
 * are drawn.<br>
    ...
    TreeNodeComponent
{
  ...
  /**
   * The Origin {@link TypeCheckerExpressionProofNode} this node represents.
   */
  ...
  /**
   * The label containing the <i>(x)</i> (the number of step) text at the
   * beginning.
   */
  ...
  /**
   * The {@link TypeComponent} showing the resulting type of this node, once the
   * node has been evaluated or the view is in advaced mode. Otherwise the
   * componet will not be shown.
   */
  private TypeComponent typeComponent;


  /**
   * The {@link TypeComponent} showing the resulting type of this node, if it is
   * a coercion
   */
   ...
   * there.<br>
   * 
   * @param pNode The origin ProofNode
   * @param model The model
   * @param pTranslator The translator of the model for the selected language
   * @param pAdvanced The advanced value
   */
  public TypeCheckerNodeComponent ( TypeCheckerProofNode pNode,
      TypeCheckerProofModel model, LanguageTranslator pTranslator,
      boolean pAdvanced )
  {
    super ();
    ...
    // if node is an expression node
    if ( pNode instanceof TypeCheckerExpressionProofNode )
    {
    ...
    this.doubleColonLabel.setForeground ( Theme.currentTheme ()
        .getExpressionColor () );
  ...
  {
    if ( this.proofNode instanceof TypeCheckerExpressionProofNode )
    {
      TypeCheckerExpressionProofNode expressionNode = ( TypeCheckerExpressionProofNode ) this.proofNode;
      this.compoundExpression.setExpression ( expressionNode.getExpression () );
      this.compoundExpression
          .setEnvironment ( expressionNode.getEnvironment () );
      this.typeComponent.setType ( expressionNode.getType () );
    }
    else
    {
      TypeCheckerTypeProofNode typeNode = ( TypeCheckerTypeProofNode ) this.proofNode;
  ...
  private void placeElements ( int pMaxWidth )
  {
    ...
    if ( this.proofNode instanceof TypeCheckerExpressionProofNode )
    {
      TypeCheckerExpressionProofNode node = ( TypeCheckerExpressionProofNode ) this.proofNode;
      // get the needed size for the expression
      Dimension expSize = this.compoundExpression.getNeededSize ( maxWidth
          - AbstractRenderer.getTextFontMetrics ().stringWidth (
              doubleColonString ) );
      this.dimension.width += expSize.width;
      this.dimension.width += AbstractRenderer.getTextFontMetrics ()
          .stringWidth ( doubleColonString );
      ...
      // the type will be renderd behind the expression if ther is enough space
      // available
      ...
      // save the psoition of the front wher the expression starts. Wher will
      // also the type start.
      int posXfront = posX;
      ...
      // set the ::
      // this.doubleColonLabel.setBounds ( posX, AbstractRenderer.getFontLeading
      // ( ), AbstractRenderer.getTextFontMetrics ( ).stringWidth (
      // doubleColonString ), expSize.height );
      this.doubleColonLabel.setBounds ( posX, 0, AbstractRenderer
          .getTextFontMetrics ().stringWidth ( doubleColonString ),
          AbstractRenderer.getAbsoluteHeight () );
      posX += this.doubleColonLabel.getSize ().width;

      // if the expression and type are not in the same line the type will be
      ...
      if ( broke )
      {
        if ( showType )
        {
          this.typeComponent.setBounds ( posXfront, 0 + expSize.height,
      ...
      else
      {
        ...
        else
        {
          ...
          this.typeEnter.setBounds ( posX,
              this.dimension.height + this.spacing, typeEnterSize.width,
              typeEnterSize.height );
    ...
    { // proofNode instance of type proof node
   ...
   * @param item
   */
  void handleMenuActivated ( JMenuItem item )
  {
