...
public class TypeCheckerNodeComponent extends JComponent implements TreeNodeComponent
{
	 ...
	 */
	public TypeCheckerNodeComponent (TypeCheckerProofNode pNode, TypeCheckerProofModel model,
			LanguageTranslator pTranslator, boolean pAdvanced)
	{
		super();
	...
	public void changeNode()
	{
		if (this.proofNode instanceof TypeCheckerExpressionProofNode)
		{
			...
			this.compoundExpression.setEnvironment(expressionNode.getEnvironment());
	...
	private void placeElements(int pMaxWidth)
	{
		...
		if (this.proofNode instanceof TypeCheckerExpressionProofNode)
		{
			...
			if ((node.getType() != null) && (node.isFinished() || this.advanced))
			{
			...
			else
			{
				...
				else
				{
					...
					this.typeEnter.setBounds(posX, this.dimension.height + this.spacing, typeEnterSize.width,
	...
	void handleTypeEntered(String type)
	{
		...
		catch (Exception e)
		{
			...
			JOptionPane.showMessageDialog(getTopLevelAncestor(), MessageFormat.format(Messages
					.getString("TypeCheckerNodeComponent.0"), type)); //$NON-NLS-1$
			return;
		}
		try
		{
