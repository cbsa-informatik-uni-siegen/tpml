    ...
    TreeNodeComponent
{
   ...
   * has been evaluated. private JLabel typeLabel;
   */
      ...
      MinimalTypingProofModel model, LanguageTranslator pTranslator )
  {
    ...
    { // proofnode is an expression proof node
    ...
    { // proof node is a type proof node
  ...
  public void changeNode ()
  {
    if ( this.proofNode.getEnvironment () != null )
    {
      ...
      this.compoundExpression
          .setEnvironment ( this.proofNode.getEnvironment () );
  ...
  private void placeElements ( int pMaxWidth )
  {
    ...
    Dimension labelComponentSize = new Dimension ( fm
        .stringWidth ( this.subTypeSymbol.getText () ), fm.getHeight () );
    this.dimension.width += labelComponentSize.width + this.spacing;

    if ( this.proofNode.getEnvironment () != null )
    { // proofNode instance of expression proof node
      ...
      this.dimension.height = Math
          .max ( typeSize.height, this.dimension.height );
    ...
    { // proofNode instance of type proof node
      ...
      this.labelComponent.setBounds ( posX, 0, labelComponentSize.width,
          this.dimension.height );
  ...
  //   
  // typeRenderer.render (typePosition, 0,typeRenderer.getNeededSize
  // (Integer.MAX_VALUE).width ,typeRenderer.getNeededSize
  // (Integer.MAX_VALUE).height, this.getGraphics (), sb, tlfmc);
  //   
  //    
  // }
  ...
  /**
   * Set the mode for this component. True means advanced, false means beginner.
   */
