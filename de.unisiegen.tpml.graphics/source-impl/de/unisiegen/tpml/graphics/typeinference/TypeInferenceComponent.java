    ...
    Scrollable, Cloneable
{
   ...
   * @param pAdvanced
   */
  public TypeInferenceComponent ( TypeInferenceProofModel pProofModel,
      boolean pAdvanced )
  {
    super ( pProofModel );
  ...
  void setAdvanced ( boolean pAdvanced )
  {
    ...
    if ( this.advanced != pAdvanced )
    {
      ...
      checkForUserObject ( ( TypeInferenceProofNode ) this.proofModel
          .getRoot () );
      ...
      while ( enumeration.hasMoreElements () )
      {
        ...
        // advanced state
        TypeInferenceProofNode node = ( TypeInferenceProofNode ) enumeration
            .nextElement ();
        TypeInferenceNodeComponent component = ( TypeInferenceNodeComponent ) node
            .getUserObject ();
  ...
  void checkForUserObject ( TypeInferenceProofNode pNode )
  {
    ...
    TypeInferenceNodeComponent nodeComponent = ( TypeInferenceNodeComponent ) pNode
        .getUserObject ();
    if ( nodeComponent == null )
    {

      // create the noded that has not been there yet
      nodeComponent = new TypeInferenceNodeComponent ( pNode, this.model,
  ...
  private void resetUserObject ( TypeInferenceProofNode node )
  {
    ...
    TypeInferenceNodeComponent nodeComponent = ( TypeInferenceNodeComponent ) node
        .getUserObject ();
    if ( nodeComponent == null )
    {
      return;
  ...
  Dimension placeNode ( TypeInferenceProofNode pNode, int pX, int pY )
  {
    ...
    while ( node != null )
    {
      ...
      // the expression is not the root node
      {
        ...
        TypeInferenceNodeComponent parentNodeComponent = ( TypeInferenceNodeComponent ) parentNode
            .getUserObject ();
  ...
  protected void doRelayout ()
  {
    ...
    TypeInferenceProofNode rootNode = ( TypeInferenceProofNode ) TypeInferenceComponent.this
    ...
    // they can be placed
    Dimension size = placeNode ( rootNode, TypeInferenceComponent.this
        .getThisBorder (), TypeInferenceComponent.this.getThisBorder () );
   ...
   * {@link #resetUserObject(TypeInferenceProofNode)} on the rootNode of the
   * model. <br>
  ...
  protected void nodesChanged ( TreeModelEvent event )
  {
    ...
    if ( children == null )
    {
      ...
      {
        TypeInferenceProofNode proofNode = ( TypeInferenceProofNode ) event
            .getPath () [ 0 ];
  ...
  void jumpToNodeVisible ()
  {
    ...
    // on the viewport
    TypeInferenceNodeComponent node = ( TypeInferenceNodeComponent ) this.jumpNode
        .getUserObject ();
    if ( node == null )
    {
      return;
