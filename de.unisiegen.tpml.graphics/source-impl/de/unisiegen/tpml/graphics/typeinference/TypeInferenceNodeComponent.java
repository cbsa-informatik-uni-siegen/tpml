...
public class TypeInferenceNodeComponent extends JComponent
{
  ...
  /**
   * The {@link TypeInferenceRulesComponent} containing the Labels of already
   ...
   * underline an expression. It is bound to every
   * {@link TypeInferenceRuleLabel} and to every {@link MenuButton}.
   */
  ...
  protected void processMouseEvent ( MouseEvent e )
  {
    ...
    try
    {
      ...
      TypeInferenceNodeComponent nextComponent = ( TypeInferenceNodeComponent ) node
          .getUserObject ();
  ...
  void menuItemActivated ( JMenuItem item )
  {
    if ( item instanceof MenuRuleItem )
    {
      ...
      try
      {
        this.proofModel.prove ( rule, this.proofNode,
            ( ( TypeInferenceComponent ) getParent () ).isAdvanced () );
    ...
    {
      // TODO this dose not function on all systems but on my system so I can
      // not find the error.
      int answer = 1;
      ...
      else
      {
        ...
        for ( int i = 0 ; i < this.proofNode.getAllFormulas ().size () ; i++ )
        {
          if ( this.proofNode.getAllFormulas ().get ( i ).getExpression () != null )
          {
            if ( this.proofModel.containsSyntacticSugar ( this.proofNode,
                this.proofNode.getAllFormulas ().get ( i ).getExpression (),
                false ) )
            {
              String [] answers =
              {
  ...
  private void freeUnderliningSibling ( boolean ignoreThis, Direction direction )
  {
    ...
    TypeInferenceNodeComponent nextNodeComponent = ( TypeInferenceNodeComponent ) nextNode
        .getUserObject ();
  ...
  private void fireRequstJumpToNode ( ProofNode node )
  {
    ...
    for ( int i = 0 ; i < listeners.length ; i += 2 )
    {
      ...
      ( ( TypeInferenceNodeListener ) listeners [ i + 1 ] )
