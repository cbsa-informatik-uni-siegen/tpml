...
public class SubTypingView extends AbstractProofView {
	...
	public SubTypingView ( SubTypingModel pSubTypingModel ) {
		...
		this.scrollPane.addComponentListener ( new ComponentAdapter ( ) {
			...
			ComponentEvent event ) {
				SubTypingView.this.component
						.setAvailableWidth ( SubTypingView.this.scrollPane.getViewport ( ).getWidth ( ) );
			}
		} );
		this.outlinePanel = new JPanel ( new GridBagLayout ( ) );

		this.outline1 = new DefaultOutline ( this );
		this.outline1.load ( this.subTypingModel.getRoot ( ).getLastLeaf ( ).getLeft ( ), Outline.ExecuteInit.SUBTYPING );
		...
		this.outline2.load ( this.subTypingModel.getRoot ( ).getLastLeaf ( ).getRight ( ), Outline.ExecuteInit.SUBTYPING );
