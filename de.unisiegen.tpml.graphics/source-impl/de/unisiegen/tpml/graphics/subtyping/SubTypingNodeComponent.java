 ...
 * row is the {@link #typeComponent2}. If the node is not completly evaluated
 * only the four dots are drawn.<br>
 ...
 * {@link #ruleButton}.That could be used to give the user the possibilty to
 * enter a type by himself.<br>
 ...
 */
public class SubTypingNodeComponent extends JComponent implements
    TreeNodeComponent
{
  ...
  public void changeNode ()
  {
    ...
    // if the node is RecSubTypingNode we have to render the A and set the
    // Tooltip
   ...
   * {@link #ruleLabel} will be placed depending whether the node is evaluated,
   * it is not evaluated or the user previously selected <i>Enter type</i>.
   * 
  ...
  private void placeElements ( int pMaxWidth )
  {
    ...
    Dimension labelComponentSize = new Dimension ( fm
        .stringWidth ( this.subTypingRecA.getText () ), fm.getHeight () );
    ...
    if ( this.proofNode instanceof RecSubTypingProofNode )
    {
      this.label.setBounds ( posX, 0, labelComponentSize.width,
          this.dimension.height );
      posX += labelComponentSize.width + this.spacing;
      this.nail.setBounds ( posX, 0, nailComponentSize.width,
          this.dimension.height );
    ...
    this.lcSubType.setBounds ( posX, 0, lcSubtypeSize.width,
        this.dimension.height );
    posX += lcSubtypeSize.width + this.spacing;
    if ( broke )
    {
      this.typeComponent2.setBounds ( posXfront, 0 + expSize.height
          + AbstractRenderer.getAbsoluteHeight (), typeSize.width,
          typeSize.height );
    }
    else
    {
  ...
  private void fireRequestJumpToNode ( ProofNode node )
  {
    ...
    for ( int i = 0 ; i < listeners.length ; i += 2 )
    {
      ...
      ( ( SubTypingNodeListener ) listeners [ i + 1 ] )
  ...
  public Dimension update ( int maxWidth )
  {
    ...
    // this.menuTranslateItem.setEnabled (
    // this.translator.containsSyntacticSugar ( this.proofNode.getExpression (
    // ),
    // true ) );
