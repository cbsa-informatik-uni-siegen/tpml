...
/**
 * Subclass of the {@link AbstractRenderer} providing the rendering of the
...
public class SubstitutionRenderer extends AbstractRenderer
{
   ...
   * <br>
   * Only the first element of the substitution is show. If there are more than
  ...
  /**
   * Containing all the informations that are not shown. When the substitution
   * is collpased.
   */
  ...
  /**
   * The String shown instaed of the rest of the substitution if it is
   * collapsed. <br>
  ...
  /**
   * Returns the information of the substitution that are not displayed.
   * 
  ...
  /**
   * Calculates the size, that is needed to propperly render the substitution.
   * 
  ...
  {
    renderer ( x, y - ( AbstractRenderer.getAbsoluteHeight () / 2 ), width,
        height, gc );
  }
  ...
  int height, Graphics gc )
  {
    ...
    if ( this.defaultTypeSubstitutionList.size () > 0 )
    {
      ...
      // if ther are more than one element in the list the rest will be a
      // tooltip.
      ...
      if ( this.defaultTypeSubstitutionList.size () > 1 )
      {
        ...
        posX += AbstractRenderer.expFontMetrics
            .stringWidth ( SubstitutionRenderer.collapsString );
