...
public class PrettyStringRenderer extends AbstractRenderer
{
  ...
  public Dimension getNeededSize ( int maxWidth )
  {
    ...
    for ( CheckerResult r : this.results )
    {
      ...
      if ( ( smallestResult == null )
          || ( smallestResult.sizeOfResult.width > r.sizeOfResult.width ) )
      {
        smallestResult = r;
      }
      ...
      {
        // the best one will be the one with the smallest heigh. everyone with
        // the same height the best one will be the smallest
        // if ( biggestResult == null || biggestResult.sizeOfResult.width <
        // r.sizeOfResult.width || biggestResult.sizeOfResult.height >
        // r.sizeOfResult.height)
        if ( ( biggestResult == null )
            || ( biggestResult.sizeOfResult.height > r.sizeOfResult.height )
            || ( ( biggestResult.sizeOfResult.height == r.sizeOfResult.height ) && ( biggestResult.sizeOfResult.width > r.sizeOfResult.width ) ) )
        {
          biggestResult = r;
        }
      }
   ...
   * given maxWidth.<br>
   * If no result fits it a new one will be calculated. First every needed
   * brakpoint will be added. If it still does not fit breakpoits will be fored.
   * If the maxWidth is to small to render a singel Char the Dimension (0, 0)
   * will be returned. <br>
   * 
   * @param maxWidth The maximum available size for this expression.
   * @return The dimensions that will be needed to perform the rendering later.
  ...
  public Dimension getNeededSizeAll_ ( int maxWidth )
  {
    ...
    // is ok to render it
    // if ther is no space available for the expression, e.g. the rules are to
    // wide
    ...
    for ( char c = it.first () ; c != CharacterIterator.DONE ; c = it.next () , indexOfActualC++ )
    {
      ...
      {
        case IDENTIFIER :
          widthOfActualC += AbstractRenderer.identifierFontMetrics
              .charWidth ( c );
          break;
        case NONE :
      ...
      if ( actualWidth > maxWidth )
      {
        ...
        // use the next breakpoint bevor the actual position. If there is no
        // breakpoint
    ...
    this.result.sizeOfResult.height = AbstractRenderer.getAbsoluteHeight ()
        * ( this.result.breakOffsets.size () + 2 );
  ...
  private CheckerResult checkLinewrap ( PrettyAnnotation annotation )
  {
    ...
    // System.out.println("Größe: "+AbstractRenderer.fontHeight + ",
    // Leading"+AbstractRenderer.fontLeading);
    ...
    for ( char c = it.first () ; c != CharacterIterator.DONE ; c = it.next () , i++ )
    {
      for ( int j = 0 ; j < resultToReturn.breakOffsets.size () ; j++ )
      {
        if ( resultToReturn.breakOffsets.get ( j ).intValue () == i )
        {
          ...
          resultToReturn.sizeOfResult.height += AbstractRenderer
              .getAbsoluteHeight ();
          resultToReturn.sizeOfResult.width = Math.max ( w,
  ...
  /**
   * Dose the same as the render mathode, but corrects the y value to render the
   * prettystring to the the baseline
   * 
   * @param x
   * @param y
   * @param width
  ...
  {
    render ( x, y - ( height / 2 ) - fontAscent / 2, width, height, gc, bound,
        toListenForM );
  }
   ...
   * @param toListenForM
   */
  public void render ( int x, int y, int width, int height, Graphics gc,
      ShowBonds bound, ToListenForMouseContainer toListenForM )
  {
    ...
    lineCount = ( ( mousePosition.y - ( posY_ - AbstractRenderer
        .getAbsoluteHeight () ) ) / AbstractRenderer.getAbsoluteHeight () ) + 1;
    ...
    if ( ( lineCount > 1 )
        && ( lineCount <= height / AbstractRenderer.getAbsoluteHeight () ) )
    {
    ...
    // rightAnnotationList will be the annotation to underline, if -1 there is
    // no underlining
    ...
    for ( char c = it.first () ; c != CharacterIterator.DONE ; c = it.next () , i++ )
    {
          ...
          && ( ( getPositionInList ( i, annotationsList ) ) > -1 ) )
      {
        ...
        this.toListenForMouse.add ( posX - 1, posYC, posX + charWidth + 1,
            posYC + fm.getAscent () );
          ...
          && ( getPositionInList ( i, annotationsList ) != -1 ) )
      {
        ...
        if ( ( getPositionInList ( i, annotationsList ) == rightAnnotationList )
            && highlight )
        {
