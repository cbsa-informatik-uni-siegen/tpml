...
/**
 * Subclass of the {@link AbstractRenderer} providing the rendering of an
 * environment.
 * 
 * @author marcell
 * @author michael
...
public class EnvironmentRenderer < S, E > extends AbstractRenderer
{
   ...
   * <br>
   * Only the first element of the environment is show. If there are more than
   * one element, they will only be shown as ", ...". Than the collapsed flag is
   * <i>true</i> else it is <i>false</i>.
   */
  ...
  /**
   * Containing all the informations that are not shown. When the environment is
   * collpased.
   */
  ...
  /**
   * Calculates the size, that is needed to propperly render the environment.
   * 
   * @return The size needed to render the environment.
   */
  public Dimension getNeededSize ()
  {
    Dimension result = new Dimension ( 2 * this.bracketSize, AbstractRenderer
        .getAbsoluteHeight () );
    ...
    else
    {
      ...
      if ( env.hasMoreElements () )
      {
        ...
        result.width += AbstractRenderer.expFontMetrics
            .stringWidth ( EnvironmentRenderer.collapsString );
      }
    }
  ...
  public void renderer ( int x, int y, int width, int height, Graphics gc )
  {
    ...
    // or else the bracket will bew renderd manually. Till now the environments
    // are shorted
    // with three dots (...) so they are never warped. To ignor the bigger size
    // made by an
    // expression (height <= AbstractRenderer.getAbsoluteHeight()) replaced by
    // if (true)
    if ( true )
    {
      gc.setFont ( expFont );
    ...
    else
    {
      ...
      gc.drawLine ( x + width - 1, y + height - 1, x + width - 1
          - this.bracketSize, y + height - 1 );
