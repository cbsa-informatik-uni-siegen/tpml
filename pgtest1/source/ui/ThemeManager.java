...
public class ThemeManager {
	...
	private Theme loadTheme (Preferences themePref) {
		Theme theme = new Theme ();
		try {
			String [] itemNames = { "Expression", "Keyword", "Constant" };
			for (int i=0; i<itemNames.length; i++) {
				Preferences itemPref = themePref.node(itemNames [i]);
				String fontName = itemPref.get("family", "Sans");
				int fontSize = itemPref.getInt ("size", 12);
				int fontStyle = itemPref.getInt("style", 0);
