...
public class Mainwindow extends JDialog {
	...
	private void initComponents ()
	{
		...
		ActionListener fileMenuListener = new ActionListener ()
		{
			...
			{
				if (e.getActionCommand () == "New") handleNew ();
				else if (e.getActionCommand () == "Save") handleSave ();
				else if (e.getActionCommand () == "Save as") handleSaveAs ();
				else if (e.getActionCommand () == "Close") handleClose ();
				else if (e.getActionCommand () ==  "Exit")  handleExit ();
			}
		};
		...
		ActionListener mlMenuListener = new ActionListener ()
		{
			public void actionPerformed (ActionEvent e)
			{
				if (e.getActionCommand () == "SmallStep") handleSmallStep ();
			}
		};
		...
		JMenu mlMenu = new JMenu ("ML Actions");
		JMenuItem smallStepItem = mlMenu.add ("SmallStep");
...
//		guiTest.closeFile (index);
	...
	private void handleSmallStep ()
	{
	    ...
	    try {
	        // Allocate the parser
	        Parser parser = new Parser(new Lexer(new PushbackReader(new StringReader(sf.getDocument().getText(0, sf.getDocument().getLength())), 1024)));
	        ...
	        SmallStepGUI gui = new SmallStepGUI(null, "narf", true, new SmallStepModel (translator.getExpression()));
	        gui.setVisible(true);
	      }
	      catch (Exception e) {
	        e.printStackTrace();
