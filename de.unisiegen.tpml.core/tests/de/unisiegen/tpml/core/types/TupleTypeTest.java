...
public class TupleTypeTest extends TestCase
{
  ...
  {
    TupleType t1 = new TupleType ( new MonoType []
    { new IntegerType (), new BooleanType () } );
    assertEquals ( "int * bool", t1.toString () );
    TupleType t2 = new TupleType ( new MonoType []
    { t1, t1 } );
    assertEquals ( "(int * bool) * (int * bool)", t2.toString () );
    TupleType t3 = new TupleType ( new MonoType []
    { t2, new ArrowType ( t1, t1 ) } );
    assertEquals (
        "((int * bool) * (int * bool)) * (int * bool \u2192 int * bool)", t3
            .toString () );
    ArrowType t4 = new ArrowType ( new ArrowType ( t1, t1 ), t1 );
    assertEquals ( "(int * bool \u2192 int * bool) \u2192 int * bool", t4
        .toString () );
  }
}
