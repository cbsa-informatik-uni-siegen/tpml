...
public class DefaultMinimalTypingTypesProofNode extends AbstractMinimalTypingProofNode implements
	...
	public PrettyStringBuilder toPrettyStringBuilder ( PrettyStringBuilderFactory pPrettyStringBuilderFactory ) {
		...
		builder.addBuilder ( this.subtype.toPrettyStringBuilder ( pPrettyStringBuilderFactory ), 0 );
		return builder;
	}
	...
	public LatexStringBuilder toLatexStringBuilder ( LatexStringBuilderFactory pLatexStringBuilderFactory, int pIndent ) {
		...
		LatexStringBuilder builder = pLatexStringBuilderFactory.newBuilder ( 0, LATEX_MINIMAL_TYPING_TYPES_PROOF_NODE, pIndent, this
				.toPrettyString ( ).toString ( ), this.subtype.toPrettyString ( ).toString ( ) );
		builder.addText ( "{" + String.valueOf ( this.getId ( ) ) + "}" ); //$NON-NLS-1$//$NON-NLS-2$
		builder.addText ( "{" + String.valueOf ( depth ) + "}" ); //$NON-NLS-1$//$NON-NLS-2$
		builder.addBuilder ( this.subtype.toLatexStringBuilder ( pLatexStringBuilderFactory, pIndent + LATEX_INDENT ), 0 );
