...
public class L1TypeCheckerProofRuleSet extends AbstractTypeCheckerProofRuleSet {
	...
	public void applyId ( TypeCheckerProofContext context, TypeCheckerProofNode pNode ) {
		TypeCheckerExpressionProofNode node = ( TypeCheckerExpressionProofNode ) pNode;
		Type type = node.getEnvironment ( ).get ( ( ( Identifier ) node.getExpression ( ) ) );
	...
	public void applyLet ( TypeCheckerProofContext context, TypeCheckerProofNode pNode ) {
		...
		if ( expression instanceof Let ) {
			...
			// add the child nodes
			context.addProofNode ( node, environment, e1, tau1 );
			context.addProofNode ( node, environment.extend ( identifiers[0], tau1 ), curriedLet.getE2 ( ), node
					.getType ( ) );
		}
	}
	...
	public void applyCoercion ( TypeCheckerProofContext context, TypeCheckerProofNode pNode ) {
		...
		context.addProofNode ( node, node.getEnvironment ( ), coercion.getE ( ), coercion.getTau1 ( ) );
