...
public abstract class AbstractLanguageScanner implements LanguageScanner
{
  ...
  //
  /**
   * Allocates a new {@link LanguageSymbol} with the specified <code>name</code>
   * and symbol <code>id</code>. The remaining parameters are optional, see
   * the documentation of the {@link LanguageSymbol} class for details.
   * 
   ...
   */
  protected final LanguageSymbol symbol ( String name , int id , int left ,
   ...
   * {@inheritDoc}
   * 
   * @see LanguageScanner#getStyleBySymbol(LanguageSymbol)
   */
  ...
  /**
   * Returns the {@link PrettyStyle} for the specified symbol <code>id</code>,
   * for example {@link PrettyStyle#KEYWORD} if the <code>id</code> refers to
   * the token <code>"let"</code>. This method must be implemented by all
   * lexers generated by JFlex.
   * 
  ...
  /**
   * Implementation of the <code>next_token()</code> interface of the
   * {@link java_cup.runtime.Scanner} interface. This is a simple wrapper around
   * the <code>nextSymbol()</code> method, which automatically skips all
   * symbols that should be ignored by the parser.
   * 
   * @return the next symbol from the scanner, skipping all tokens that should
   *         be ignored by the parser, or <code>null</code> on end-of-file.
   * @see java_cup.runtime.Scanner#next_token()
   * @throws IOException if an error occurs while reading characters from the
   *           input source stream.
