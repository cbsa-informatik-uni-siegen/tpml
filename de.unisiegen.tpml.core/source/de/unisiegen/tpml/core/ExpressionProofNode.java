...
/**
 * Base interface for special proof nodes that have an expression associated
 * with it and present the fundamental parts of the
...
public interface ExpressionProofNode extends ProofNode
{
  ...
  /**
   * Returns the {@link Expression} associated with this proof node. This is
   * garantied to never return <code>null</code>.
   * 
  ...
  /**
   * Returns the {@link ProofStep}s which were already performed on this proof
   ...
   * applied.
   * 
   * @return the {@link ProofStep}s or an empty array if no rules were applied
   *         to this node yet.
   * @see #getExpression()
   * @see #getRules()
   */
