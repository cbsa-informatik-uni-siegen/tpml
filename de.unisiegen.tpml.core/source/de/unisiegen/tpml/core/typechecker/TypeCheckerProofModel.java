...
public class TypeCheckerProofModel extends AbstractExpressionProofModel {
	 ...
	 */
	private void applyInternal ( TypeCheckerProofRule rule, AbstractTypeCheckerProofNode node, MonoType type )
	...
	void contextApplySubstitution ( DefaultTypeCheckerProofContext context, TypeSubstitution s ) {
		...
		while ( nodes.hasMoreElements ( ) ) {
			...
			final AbstractTypeCheckerProofNode nextNode = ( AbstractTypeCheckerProofNode ) nodes.nextElement ( );
			if ( nextNode instanceof DefaultTypeCheckerExpressionProofNode ) {
				...
				if ( !oldEnvironment.equals ( newEnvironment ) || !oldExpression.equals ( newExpression )
	...
	public LatexInstructionList getLatexInstructions ( ) {
		...
		instructions.add ( getLatexInstructionsInternal ( ( TypeCheckerProofNode ) this.root ) );
		return instructions;
	}
	 ...
	 *         {@link ProofNode}.
	 */
	private LatexInstructionList getLatexInstructionsInternal ( TypeCheckerProofNode pNode ) {
		LatexInstructionList instructions = new LatexInstructionList ( );
	...
	public void prove ( ProofRule rule, ProofNode node ) throws ProofRuleException {
		...
		// try to apply the rule to the specified node
		applyInternal ( ( TypeCheckerProofRule ) rule, ( AbstractTypeCheckerProofNode ) node, null );
	}
