...
public interface MinimalTypingProofContext
{
   ...
   * 
   * @param node the parent node.
   * @param type the first type for this node
   * @param type2 the second type for this node
   * @throws IllegalArgumentException if the <code>node</code> is invalid for
   *           the proof model that is associated with this proof context.
   * @throws NullPointerException if any of the parameters is <code>null</code>.
   */
  ...
  /**
   * Set the type for a proofnode
   * 
   * @param node the proofnode the type should be set
   * @param type the new mono type for this proof node
   */
  ...
  /**
   * Add a new seen type for the proof node
   * 
