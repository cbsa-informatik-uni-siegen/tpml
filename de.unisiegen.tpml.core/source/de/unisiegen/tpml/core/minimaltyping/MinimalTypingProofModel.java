...
public class MinimalTypingProofModel extends AbstractExpressionProofModel {
	 ...
	 */
	private void applyInternal ( MinimalTypingProofRule rule, MinimalTypingProofNode node, MonoType type )
	 ...
	 */
	public void contextAddProofNode ( DefaultMinimalTypingProofContext context,
			final AbstractMinimalTypingProofNode node, TypeEnvironment environment, Expression expression ) {
		if ( context == null ) {
			throw new NullPointerException ( "context is null" ); //$NON-NLS-1$
		}
		if ( node == null ) {
		...
		final DefaultMinimalTypingExpressionProofNode child = new DefaultMinimalTypingExpressionProofNode ( environment,
				expression );
		context.addRedoAction ( new Runnable ( ) {
			@SuppressWarnings ( "synthetic-access" )
			public void run ( ) {
	 ...
	 */
	public void contextSetProofNodeType ( DefaultMinimalTypingProofContext context,
	...
	public TreeSet < LatexCommand > getLatexCommands ( ) {
		...
		for ( LatexCommand command : getLatexCommandsInternal ( ( MinimalTypingProofNode ) this.root ) ) {
			commands.add ( command );
		}
		return commands;
	}
	 ...
	 *         {@link ProofNode}.
	 */
	private TreeSet < LatexCommand > getLatexCommandsInternal ( MinimalTypingProofNode pNode ) {
		TreeSet < LatexCommand > commands = new TreeSet < LatexCommand > ( );
	...
	public LatexInstructionList getLatexInstructions ( ) {
		...
		instructions.add ( getLatexInstructionsInternal ( ( MinimalTypingProofNode ) this.root ) );
		return instructions;
	}
	 ...
	 *         {@link ProofNode}.
	 */
	private LatexInstructionList getLatexInstructionsInternal ( MinimalTypingProofNode pNode ) {
		LatexInstructionList instructions = new LatexInstructionList ( );
		...
		for ( int i = 0; i < pNode.getChildCount ( ); i++ ) {
			instructions.add ( getLatexInstructionsInternal ( pNode.getChildAt ( i ) ) );
		}
		return instructions;
	}
	 ...
	 *         {@link ProofNode}.
	 */
	private TreeSet < LatexPackage > getLatexPackagesInternal ( MinimalTypingProofNode pNode ) {
		TreeSet < LatexPackage > packages = new TreeSet < LatexPackage > ( );
